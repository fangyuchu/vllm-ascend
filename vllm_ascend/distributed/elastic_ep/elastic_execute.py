...
def batch_transfer_weights(
    ...
    for index, param in enumerate(all_params):
        if is_sender:
            worker = dp_group.device_group.send([param], peer_rank, tag=index)
        else:
            worker = dp_group.device_group.recv([param], peer_rank, tag=index)
...
class AscendElasticEPScalingExecutor(ElasticEPScalingExecutor):
    ...
    def create_standby_groups(self, reconfig_request: ReconfigureDistributedRequest) -> None:
        super().create_standby_groups(reconfig_request)
        ...
        create_ascend_standby_groups(
            ...
            group_ports=reconfig_request.new_platform_stateless_ports,
        )
    ...
    def switch_and_prepare(self) -> None:
        ...
        for module in moe_modules:
            ...
            if self.worker.model_config.quantization is not None:

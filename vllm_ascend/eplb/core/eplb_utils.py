...
def reinit_expert(local_expert_map, num_moe_layers, layer_id):
    ...
    if _GLOBAL_PLACEMENT is not None:
        ...
        for rankid in range(world_size):
            ...
            local_placement = global_placement[rankid]
            local_expert_map[local_placement] = torch.arange(local_placement.shape[0],
                                                             dtype=torch.int32)
...
def generate_log2phy_map(expert_map, sync=False):
    ...
    row_indices = torch.arange(world_size, device=device).view(-1, 1).expand(world_size,
    ...
    sorted_vals, _ = log2phy_map.sort(dim=0, stable=True,
                                      descending=True)

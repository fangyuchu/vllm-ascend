...
class TokenDispatcherWithMC2(MoETokenDispatcher):
    ...
    def __init__(self, **kwargs):
        ...
        local_rank = get_mc2_group().rank_in_group if parallel_config.enable_stateless_pg else torch.distributed.get_rank(group=device_group)
...
class TokenDispatcherWithAll2AllV(MoETokenDispatcher):
    ...
    def __init__(self, **kwargs):
        ...
        from vllm.config import get_current_vllm_config
        parallel_config = get_current_vllm_config().parallel_config
        local_rank = self.ep_rank if parallel_config.enable_stateless_pg else torch.distributed.get_rank(group=self.ep_group)

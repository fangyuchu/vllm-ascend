...
from torch.distributed.distributed_c10d import ProcessGroup, PrefixStore, _get_default_timeout
...
def create_stateless_process_group(
    ...
    logger.info(f"Successfully created HCCL process group for ranks {ranks},rank {rank} "
...
def _initialize_process_group(
    ...
    backend_class = ProcessGroupHCCL(
        prefix_store, group_rank, group_size, hccl_pg_options
    )

...
from vllm.config import CUDAGraphMode, VllmConfig, set_current_vllm_config, get_current_vllm_config
...
from vllm_ascend.distributed.parallel_state import init_ascend_model_parallel, get_elastic_info
...
from vllm_ascend.worker.descale import (destory_acl_graph, rebuild_acl_graph, destory_comm_group,
                                        reinit_comm_group, save_expert_weights_to_ram, expand_expert_weights,
                                        reload_fault_expert_weights, update_parallel_config, init_ep2dp_map,
                                        update_ep2dp_map, update_elastic_info, reconfigure_moe,
                                        get_expert_distribution_after_descale, gen_global_log2phy_map,
                                        init_global_expert_distribution, generate_redundant_expert_ids,
                                        gen_local_log2phy_map, gen_expert_backup_map)
...
class NPUWorker(WorkerBase):
    ...
    def __init__(
        ...
        ep_size = self.vllm_config.parallel_config.data_parallel_size * self.vllm_config.parallel_config.tensor_parallel_size
        self.ep2dp_map = init_ep2dp_map(self.vllm_config.parallel_config.data_parallel_size,
                                        self.vllm_config.parallel_config.tensor_parallel_size)
        self.experts_saved_ids = []
        self.experts_saved_weights = {}
        self.quant = True if self.model_config.quantization is not None else False
        ...
        if num_redundancy_expert and get_ascend_device_type() in AscendDeviceType.A3:
            ...
            redundant_expert_list = generate_redundant_expert_ids(self.num_logical_expert, ep_size, num_redundancy_expert)
    ...
    def dp_descale(self, exclude_dp_ranks, vllm_update_config):
        ...
        self.global_log2phy_map, redistributed_experts, added_experts, replaced_redundant_experts, self.use_mask_mc2 = get_expert_distribution_after_descale(
            exclude_dp_ranks, self.experts_distribution, self.global_log2phy_map, self.backup_expert_rank_mapping, self.use_mask_mc2)
        ...
        if rank not in exclude_dp_ranks:
            self.experts_saved_ids, self.experts_saved_weights = save_expert_weights_to_ram(expert_ids_to_save,
                                                                                            self.experts_saved_ids,
                                                                                            self.experts_saved_weights,
                                                                                            self.vllm_config,
                                                                                            self.model_runner,
            ...
            reload_fault_expert_weights(self.model_runner, self.global_experts_distribution,
                                        self.experts_saved_weights, self.vllm_config,
                                        redistributed_experts, added_experts, replaced_redundant_experts, self.quant)
            ...
            reconfigure_moe(self.use_mask_mc2, self.model_runner, self.vllm_config, num_logical_expert,
                            num_new_phy_experts, self.log2phy)

...
from vllm.distributed.parallel_state import get_ep_group, get_dp_group, get_pp_group, get_pcp_group, get_tp_group
...
class NPUWorker(WorkerBase):
    ...
    def _reshard_experts(self):
        ...
        while eplb_updator.update_info_all:
            (expert_send_info, expert_recv_info, updated_expert_map,
             log2phy_map, layer_id) = eplb_updator.update_info_all.pop(0)
    ...
    def _eplb_after_scale_up(self, old_ep_size: int, new_ep_size: int) -> None:
        ...
        self.model_runner.eplb_updator.broad_cast_global_placement()
        torch_npu.npu.synchronize()
        eplb_updator.cur_iterations = 0
    ...
    def reconfigure_moe(
        ...
        moe_modules = [
            ...
            if (module.__class__.__name__ == "AscendFusedMoE"
                or module.__class__.__name__ == "AscendSharedFusedMoE")
        ]
    ...
    def reinitialize_distributed(
        ...
        from vllm.distributed.parallel_state import destroy_ascend_model_parallel
        ...
        shutdown_ray = True if self.parallel_config.data_parallel_backend == "ray" else False
        destroy_ascend_model_parallel()
        ...
        self.broadcast_num_redundant_experts(num_redundant_experts)

        self.reconfig_moe(old_ep_size, new_ep_size, num_logical_experts,
                          num_logical_experts + num_redundant_experts)

        if new_ep_size > old_ep_size:
            self._eplb_after_sacle_up(old_ep_size, new_ep_size)
    ...
    def broadcast_num_redundant_experts(self, num_redundancy_experts=-1):

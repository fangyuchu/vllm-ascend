...
Per_device_card = 2
lib = cdll.LoadLibrary("/usr/local/dcmi/libdcmi.so")
...
def start_monitor_engine_status(host, port, timeout, external_fault_notify_port):
    ...
    while True:
        ...
        for x in exclude_dp_ranks:
            active_npus.remove(get_npu_by_dp(x))
    ...
    while True:
        ...
        for device in device_list:
            ...
            if CardDropFaultCode in result[0]:
                ...
                descale_npu.update([npu for npu in error_npu if npu in active_npus])
